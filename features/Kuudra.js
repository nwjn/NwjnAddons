...
let phase = 0
let [s1, s2, s3, s4, s5, s6] = [true, true, true, true, true, true]
let [freshies, freshTime, build] = [[], 0, 0]
...
registerWhen(register("chat", (player) => {
  // TODO: TEST
  player.includes("]") && (player = player.substring(player.indexOf(" ") + 1))
  player.includes(" ") && (player = player.substring(0, player.indexOf(" ")))
...
registerWhen(register("renderEntity", (stand) => {
  if (stand.getClassName() != "EntityArmorStand" || phase != 1 || !stand.getName().includes("SUPPLIES RECEIVED")) return
  
  const x = ~~stand.getX()
  const z = ~~stand.getZ()
  if (x == -98 && z == -112) s1 = false
  else if (x == -98 && z == -99) s2 = false
  else if (x == -110 && z == -106) s3 = false
  else if (x == -106 && z == -112) s4 = false
  else if (x == -94 && z == -106) s5 = false
  else if (x == -106 && z == -99) s6 = false
...
}).setCriteria("Your Fresh Tools Perk bonus doubles your building speed for the next 10 seconds!"), () => getWorld() == "Kuudra" && (settings.inBuild || settings.fresh))
...
registerWhen(register("chat", (supply, event) => {
  // ? why didnt i use ChatLib.getChatMessage(event, true)
...
registerWhen(register("renderWorld", () => {
  ...
  if (settings.pearl) {
    // TODO: Use render methods to make these look better
    // Triangle
  ...
  if (settings.supplyWaypoints && phase == 1) {
    // TODO: TEST
    const giants = World.getAllEntitiesOfType(EntityGiant.class).filter(e => e.getEntity().func_70694_bm()?.toString() == "1xitem.skull@3")
    giants.forEach(giant => {
      const yaw = giant.getYaw();
      renderBeaconBeam(giant.getRenderX() + (3.7 * Math.cos((yaw + 130) * (Math.PI / 180))), 72, giant.getRenderZ() + (3.7 * Math.sin((yaw + 130) * (Math.PI / 180))), 0, 1, 1, 0.8, true, 100);
    });
  }
...
registerWhen(register("renderWorld", () => {
  World.getAllEntitiesOfType(EntityMagmaCube.class).forEach(entity => {
    let maxHP = entity.getEntity().func_110148_a(SMA.field_111267_a).func_111125_b()
    if (maxHP == 100_000) {
      RenderLib.drawEspBox(entity.getRenderX(), entity.getRenderY(), entity.getRenderZ(), entity.getWidth(), entity.getHeight(), 0.914, 1, 0, 1, true)
    }
  })
  /* 
  TODO (TEST & REPLACE)
  ...
  ~boss && RenderLib.drawEspBox(boss.getRenderX(), boss.getRenderY(), boss.getRenderZ(), boss.getWidth(), boss.getHeight(), 0.914, 1, 0, 1, true)
